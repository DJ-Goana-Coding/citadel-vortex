...
def _safe_host_env(default: str = "0.0.0.0") -> str:
    raw_value = os.getenv("HOST")
    if raw_value is None:
        return default

    allowed_hosts = {"0.0.0.0", "127.0.0.1", "localhost", "::", "::1"}
    if raw_value in allowed_hosts:
        return raw_value

    logger.warning("Invalid HOST=%r; falling back to %s", raw_value, default)
    return default
